...
int catimg_main(int argc, char *argv[])
{
...
#if defined(FEAT_IOS)
        // use precision 2 by default, since ios supports unicode
        precision = 2;
#else
        if (supportsUTF8())
            precision = 2;
...
#if defined(FEAT_IOS)
    if (cols < 1) {
        // terminal_columns() usually fails,
        // use env variable $COLUMNS,
        // and the last resort is to set it to 80
        if (cols < 1) {
            const char *COLUMNS = getenv("COLUMNS");
            cols = (int)strtol(COLUMNS, NULL, 10);
            if (cols == 0) {
                cols = 80;
            }
        }
        cols = cols / (2 / precision);
    }
#endif
    ...
    if (img.frames > 1) {
#if defined(FEAT_IOS)
        // "clear" not available, use ANSI code instead
        printf("\e[H\e[2J");
#else
        system("clear");
#endif
